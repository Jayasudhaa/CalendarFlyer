...
app.post('/api/translate', async (req, res) => {
  ...
  if (!key) return res.status(500).json({ error: 'GOOGLE_TRANSLATE_API_KEY not configured' });
  try {
      ...
      const reqHttp = https.request({
        ...
        r.on('end', () => {
          try {
            ...
            if (r.statusCode !== 200) return reject(new Error(d?.error?.message || 'Translation failed'));
            resolve(d?.data?.translations?.[0]?.translatedText || '');
          } catch(e) { reject(e); }
        });
      });
