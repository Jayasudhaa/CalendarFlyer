  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (raw) {
        ...
        catch { throw new Error(`Server returned an unexpected response (status ${response.status}). Check that the backend server is running.`); }
      } else if (!response.ok) {
        throw new Error(`Server error (status ${response.status}) with no response body. Check the backend terminal for a crash log.`);
      }
