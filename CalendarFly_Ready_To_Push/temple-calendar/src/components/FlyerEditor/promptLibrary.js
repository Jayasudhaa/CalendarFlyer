...
const PROMPT_LIBRARY = [
  {
    ...
    name: "🕉️ Abhishekam (Sacred Bathing Ritual)",
  ...
  {
    keywords: ['ganapathi', 'ganesh', 'ganesha', 'ganpati', 'vinayaka', 'chaturthi', 'siddhi', 'ganeshotsav', 'ganesh utsav', 'ganeshotsava', 'ganapati utsav', 'vinayagar'],
  ...
  {
    keywords: ['lakshmi', 'kojagara', 'diwali', 'deepavali', 'deepawali', 'dhanteras'],
    name: "💰 Diwali / Deepavali (Goddess Lakshmi)",
  ...
  {
    keywords: ['varalakshmi', 'varamahalakshmi', 'vara lakshmi'],
    name: "🪔 Varalakshmi Vratam",
    prompt: `Varalakshmi Vratam altar — a decorated brass kalasha (sacred pot) filled with rice and topped with fresh mango leaves and a coconut, dressed as Goddess Lakshmi with an ornate silver or gold face mask (mukha) attached to the front, draped in a red and gold silk saree with a garland of fresh flowers around the neck, heavy gold jewelry adorning the kalasha, sacred thread (thoranam) tied around, surrounded by turmeric-smeared betel leaves, areca nuts, a small oil lamp (deepam) glowing beside it, kolam/rangoli with lotus motifs on the floor beneath, marigold and jasmine flowers scattered around, warm golden festive light, deeply devotional South Indian home-shrine atmosphere`,
  },
  {
    keywords: ['ekadashi', 'ekadasi'],
    name: "🌙 Ekadashi (Lord Vishnu Fasting Day)",
    prompt: `Lord Vishnu reclining serenely on the coils of the great serpent Adishesha in the cosmic ocean of milk (Kshira Sagara), four arms holding conch (shankha), discus (chakra), mace (gada) and lotus, wearing golden crown and yellow silk pitambaram, Goddess Lakshmi gently massaging his feet, a fresh tulsi (holy basil) plant with garland in the foreground symbolizing the Ekadashi fast, soft blue and gold color palette, serene cosmic starlit background, gentle divine glow, peaceful contemplative atmosphere`,
  },
  {
    keywords: ['shiva', 'shivaratri', 'rudra', 'mahashivratri', 'shiv'],
    name: "🔱 Maha Shivaratri (Lord Shiva)",
  ...
  {
    keywords: ['durga', 'navaratri', 'navratri', 'devi', 'shakti', 'ambika', 'kali', 'chamundi', 'dasara', 'dussehra', 'vijayadashami', 'sharannavratri', 'golu', 'kolu'],
    name: "⚔️ Navaratri / Dasara (Goddess Durga)",
  ...
  },
  {
    ...
    name: "📿 Saraswati Puja / Vidyarambham",
...
/**
 * List of { name } options for a deity/festival picker dropdown —
 * lets the user manually pick a prompt instead of relying on keyword auto-match.
 */
export const PROMPT_LIBRARY_OPTIONS = PROMPT_LIBRARY.map(p => p.name);

/**
 * Returns the full prompt text (base prompt + style suffix) for a given
 * deity/festival name from the dropdown. The result is meant to be editable —
 * the caller drops it straight into the prompt textarea.
 */
export const getPromptByName = (name) => {
  const match = PROMPT_LIBRARY.find(p => p.name === name);
  if (!match) return '';
  return `${match.prompt}. ${BASE_STYLE}`;
};
