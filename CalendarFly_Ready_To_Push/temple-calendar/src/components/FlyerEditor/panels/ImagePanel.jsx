...
export default function ImagePanel({
  ...
  return (
    <div>
      ...
      {imageTab === 'ai' && (
        <div>
          <span style={sL}>Deity / Festival</span>
          <select
            value={selectedDeityName}
            onChange={e => {
              setSelectedDeityName(e.target.value);
              if (e.target.value) setGenPrompt(getPromptByName(e.target.value));
            }}
            style={{ ...inp, marginBottom: 10, cursor: 'pointer' }}
          >
            <option value="" disabled>Choose to auto-fill prompt below…</option>
            {PROMPT_LIBRARY_OPTIONS.map(name => (
          ...
          <div style={{ color: '#64748b', fontSize: '0.68rem', marginBottom: 12 }}>Auto-generated from event type, or pick a deity/festival above. Edit freely to customize.</div>
