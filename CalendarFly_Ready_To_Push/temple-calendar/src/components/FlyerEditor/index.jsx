...
import { buildFlyer, placeDeityImage as _placeImage, loadFlyerFonts, getFontForLang } from './canvasBuilder';
...
export default function FlyerEditor({ event, onClose }) {
  ...
  useEffect(() => {
    ...
    loadFlyerFonts(activeLang).then(() => {
      buildFlyer({ canvas, dims, theme: t, event, onSelectionChange: setSelectedObject, lang: activeLang });
    });
  ...
  useEffect(() => {
    ...
    // 1. Save outgoing layout state
    if (prevKey && prevKey !== layout) {
      layoutStates.current[prevKey] = {
        json: canvas.toJSON([
        'name', 'selectable', 'evented', 'originX', 'originY',
        'strokeDashArray', 'lineHeight', 'cropX', 'cropY', '_originalText',
        ]),
    ...
    const saved = layoutStates.current[layout];
    if (saved?.json) {
  ...
  const handleTranslate = async (langCode, silent = false) => {
    ...
    const fontMap = { hi: '"Noto Sans Devanagari"', ta: '"Noto Sans Tamil"', te: '"Noto Sans Telugu"', kn: '"Noto Sans Kannada"', en: 'Georgia' };
    const fontFamily = fontMap[langCode] || 'Georgia';
    ...
    for (const obj of objs) {
      ...
      obj.set({ text: translated, fontFamily });
    }
    // Save state for layout switching
    const currentKey = cv._currentLayoutKey || layout;
    layoutStates.current[currentKey] = {
      json: cv.toJSON(['name','selectable','evented','originX','originY','strokeDashArray','lineHeight','cropX','cropY','_originalText']),
      imgUrl: placedImgUrl.current,
    };
  ...
  return (
    <>
      <div style={{ position: 'fixed', inset: 0, zIndex: 9999, background: '#1a0e04', display: 'flex', flexDirection: 'column', fontFamily: 'Georgia,serif' }}>

        {/* ═══ TOP TOOLBAR ═══ */}
        <div style={{ height: 58, background: '#231206', borderBottom: '1px solid #3a2008', display: 'flex', alignItems: 'center', padding: '0 12px', gap: 6, flexShrink: 0, overflowX: 'auto' }}>
          <div style={{ background: '#c2410c', borderRadius: 7, padding: '6px 12px', color: 'white', fontWeight: '700', fontSize: '0.8rem', letterSpacing: '0.04em', flexShrink: 0, fontFamily: 'Georgia, serif' }}>
            🪔 Flyer Studio
          </div>

          <span style={{ color: '#c9943a', fontSize: '0.78rem', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', maxWidth: 150, fontFamily: 'Georgia, serif' }}>
            {event?.title || 'New Flyer'}
          </span>

          <div style={{ width: 1, height: 28, background: '#3a2008', margin: '0 3px', flexShrink: 0 }} />
            ...
            {LANGUAGES.map(lang => {
              ...
              <button key={lang.code} onClick={() => handleTranslate(lang.code)} disabled={translating} style={{
                padding: '5px 10px',
                background: active ? '#c2410c' : '#1a0e04',
                color: active ? '#fff' : '#c9943a', borderRadius: 6, cursor: 'pointer',
                border: `1.5px solid ${active ? '#fb923c' : '#3a2008'}`,
                fontSize: '0.76rem', fontFamily: 'Georgia, serif', fontWeight: active ? '700' : '400',
              }}>{lang.label}</button>
              );
            })}
            ...
            {Object.entries(LAYOUTS).map(([key, l]) => {
              ...
              return (
                <button key={key} onClick={() => {
                  ...
                  layoutStates.current[layout] = fabricRef.current.toJSON(['name','selectable','evented','originX','originY','strokeDashArray','lineHeight','cropX','cropY']);
                    fabricRef.current._currentLayoutKey = key;
                  }
                  setLayout(key);
                }} style={{
                padding: '5px 10px', border: `1.5px solid ${active ? c : '#334155'}`,
                borderRadius: 6, cursor: 'pointer', fontSize: '0.76rem',
                background: active ? c + '33' : '#1a0e04',
                color: active ? '#fff' : '#c9943a',
                fontFamily: 'Georgia, serif', fontWeight: active ? '700' : '400',
              }}>{l.label} <span style={{ opacity: 0.6, fontSize: '0.65rem' }}>{l.desc}</span></button>
              );
            })}

          <div style={{ flex: 1 }} />

          <button onClick={() => setShowHistory(true)} style={{ padding: '5px 11px', background: '#1a0e04', border: '1.5px solid #3a2008', color: '#c9943a', borderRadius: 6, cursor: 'pointer', fontSize: '0.76rem', fontFamily: 'Georgia, serif' }}>🕐 History</button>
          <button onClick={handleSaveDraft} disabled={saving} style={{ padding: '5px 11px', background: '#1a0e04', border: '1.5px solid #3a2008', color: '#c9943a', borderRadius: 6, cursor: 'pointer', fontSize: '0.76rem', fontFamily: 'Georgia, serif' }}>💾 {saving ? 'Saving…' : 'Save Draft'}</button>
          <button onClick={handleSaveToS3} disabled={uploading} style={{ padding: '5px 11px', background: '#1a0e04', border: '1.5px solid #3a2008', color: '#c9943a', borderRadius: 6, cursor: 'pointer', fontSize: '0.76rem', fontFamily: 'Georgia, serif' }}>☁ {uploading ? 'Uploading…' : 'Save to Cloud'}</button>
          <button onClick={handleDownload} disabled={downloading} style={{ padding: '5px 14px', background: '#16a34a', border: '1.5px solid #16a34a', color: 'white', borderRadius: 6, cursor: downloading ? 'not-allowed' : 'pointer', fontWeight: '700', fontSize: '0.76rem', fontFamily: 'Georgia, serif' }}>⬇ Download</button>
          <button onClick={() => setShowBroadcast(true)} style={{ padding: '5px 14px', background: '#6d28d9', border: '1.5px solid #7c3aed', color: 'white', borderRadius: 6, cursor: 'pointer', fontWeight: '700', fontSize: '0.76rem', fontFamily: 'Georgia, serif', whiteSpace: 'nowrap' }}>📣 Broadcast</button>





          <div style={{ width: 1, height: 28, background: '#3a2008', margin: '0 2px', flexShrink: 0 }} />
          <button onClick={onClose} style={{ width: 32, height: 32, background: '#1a0e04', border: '1.5px solid #3a2008', color: '#c9943a', borderRadius: 6, cursor: 'pointer', fontSize: '0.85rem', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>✕</button>

        </div>

        {/* Toast */}
        {toast && (
          <div style={{ position: 'fixed', bottom: 24, right: 24, zIndex: 999999, background: '#231206', border: '1px solid #c9943a', color: toast.startsWith('⚠') ? '#f87171' : '#34d399', padding: '10px 20px', borderRadius: 10, fontSize: '0.85rem', fontWeight: '600', boxShadow: '0 8px 24px rgba(0,0,0,0.5)' }}>
            {toast}
          </div>
        )}
        ...
        <div style={{ flex: 1, display: 'flex', overflow: 'hidden' }}>
          ...
          {/* Icon rail */}
          <div style={{ width: 60, background: '#1a0e04', borderRight: '1px solid #3a2008', display: 'flex', flexDirection: 'column', alignItems: 'center', paddingTop: 10, gap: 2, flexShrink: 0 }}>
            {PANELS.map(p => (
              <button key={p.id} onClick={() => setActivePanel(p.id)} title={p.label} style={{
                ...
                background: activePanel === p.id ? 'rgba(194,65,12,0.25)' : 'transparent',
                color: activePanel === p.id ? '#fb923c' : '#c9943a',
          ...
          {/* Sidebar panel */}
          <div style={{ width: 248, background: '#160b02', borderRight: '1px solid #3a2008', overflowY: 'auto', flexShrink: 0 }}>
          ...
          {/* Canvas area */}
          <div style={{ flex: 1, overflow: 'auto', display: 'flex', flexDirection: 'column', alignItems: 'center', padding: 24, background: '#231206', position: 'relative' }}>
            {/* Zoom controls */}
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 16, background: '#1a0e04', border: '1px solid #3a2008', borderRadius: 10, padding: '5px 12px' }}>
              <button onClick={() => setZoom(z => Math.max(z - 0.1, 0.2))} style={{ background: 'none', border: 'none', color: '#c9943a', cursor: 'pointer', fontSize: '1.1rem', padding: '0 4px' }}>−</button>
              <span style={{ color: '#c9943a', fontSize: '0.78rem', minWidth: 42, textAlign: 'center' }}>{Math.round(zoom * 100)}%</span>
              <button onClick={() => setZoom(z => Math.min(z + 0.1, 1.5))} style={{ background: 'none', border: 'none', color: '#c9943a', cursor: 'pointer', fontSize: '1.1rem', padding: '0 4px' }}>+</button>
              <button onClick={() => setZoom(Math.min((window.innerWidth - 340) / dims.w, (window.innerHeight - 140) / dims.h, 1))} style={{ background: 'none', border: 'none', color: '#7a5a30', cursor: 'pointer', fontSize: '0.7rem' }}>Fit</button>
            </div>
