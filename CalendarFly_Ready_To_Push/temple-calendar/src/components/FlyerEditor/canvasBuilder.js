...
export function getFontForLang(lang = 'en') {
  ...
  return map[lang] || 'Georgia';
}
/**
 * Draws the full flyer layout onto a Fabric.js canvas.
 * Called whenever layout or theme changes.
 */
export const buildFlyer = ({ canvas, dims, theme: t, event, onSelectionChange, lang = 'en' }) => {
  const { w, h } = dims;
  const cx = w / 2;
  const F  = getFontForLang(lang); // font family for translated text
  const FE = "Georgia, serif";     // always English for temple name/address
  ...
  canvas.add(new window.fabric.Textbox((event?.title || 'EVENT TITLE').toUpperCase(), {
    ...
    textAlign: 'center', fontFamily: F, fontSize: Math.round(w * 0.056),
  ...
  canvas.add(new window.fabric.Textbox('CELEBRATION', {
    ...
    textAlign: 'center', fontFamily: F, fontSize: Math.round(w * 0.018),
  ...
  canvas.add(new window.fabric.Textbox(dateStr, {
    ...
    textAlign: 'center', fontFamily: F, fontSize: Math.round(w * 0.030),
  ...
  if (event?.time) {
    canvas.add(new window.fabric.Textbox(event.time, {
      ...
      textAlign: 'center', fontFamily: F, fontSize: Math.round(w * 0.022),
  ...
  if (event?.description) {
    canvas.add(new window.fabric.Textbox(event.description, {
      ...
      textAlign: 'center', fontFamily: F, fontSize: Math.round(w * 0.020),
  ...
  canvas.add(new window.fabric.Textbox(`RSVP: ${rsvpUrl}`, {
    ...
    textAlign: 'center', fontFamily: F, fontSize: Math.round(w * 0.016),
