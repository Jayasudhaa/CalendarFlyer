    ...
    <div style={S.wrap}>
      <style>{`
        @media (max-width: 640px) {
          .cf-analytics-body { grid-template-columns: 1fr !important; overflow: visible !important; }
          .cf-analytics-sidebar { max-height: 200px !important; border-right: none !important; border-bottom: 1px solid var(--cf-border) !important; }
          .cf-analytics-content { overflow-y: visible !important; }
          .cf-stat-grid-4 { grid-template-columns: repeat(2,1fr) !important; }
          .cf-stat-grid-3 { grid-template-columns: repeat(2,1fr) !important; }
          .cf-analytics-tabs button { font-size: 0.72rem !important; padding: 7px 6px !important; gap: 3px !important; }
          .cf-analytics-tabs span { display: none !important; }
          .cf-analytics-padding { padding: 12px !important; }
        }
      `}</style>
      <div style={S.header}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          <TrendingUp style={{ width: 22, height: 22, color: '#b97a3a' }} />
          <div>
      ...
      <div className="cf-analytics-body" style={S.body}>
        <div className="cf-analytics-sidebar" style={S.sidebar}>
        ...
        <div
          className="cf-analytics-content"
          style={{
            flex: 1,
            minWidth: 0,
            display: 'flex',
          ...
          {activeTab === 'monthly' && (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
              ...
              {!monthLoading && monthlyData.length > 0 && (
                <div className="cf-stat-grid-4" style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 12 }}>
                  {[
                ...
                {monthLoading ? (
                  ...
                  <div style={{ overflowX: 'auto' }}>
                    <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                      <thead>
                        <tr style={{ borderBottom: '2px solid #e8d5b7' }}>
                          ...
                          .map(({ event, rsvp }) => {
                            ...
                            return (
                              <tr
                                ...
                                onClick={() => handleEventSelect(event)}
                              >
                                <td style={{ ...S.td, color: '#b97a3a', whiteSpace: 'nowrap' }}>{dateStr}</td>
                                <td style={{ ...S.td, fontWeight: 600, color: '#3d2008', maxWidth: 180 }}>
                      ...
                      <tfoot>
                        <tr style={{ borderTop: '2px solid #e8d5b7', background: 'linear-gradient(135deg,#fffdf7,#fff8ee)' }}>
                          <td style={S.td} />
                          <td style={{ ...S.td, fontWeight: 700, color: '#3d2008' }}>
              ...
              {!selectedEvent ? (
                ...
                <>
                  <div style={{ ...S.card, padding: '14px 18px' }}>
                    <div style={{ fontWeight: 700, color: '#3d2008', fontSize: '0.95rem' }}>{selectedEvent.title}</div>
                  ...
                  {detailLoading ? (
                    ...
                    <>
                      ...
                      <div style={S.card}>
                        ...
                        <div className="cf-stat-grid-3" style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 12, marginTop: 14 }}>
                          {[
                            ...
                            <div key={b.label} style={{ background: 'linear-gradient(135deg,#fffdf7,#fff8ee)', borderRadius: 8, padding: 14 }}>
                              <div style={{ fontSize: '0.82rem', fontWeight: 600, color: b.color, marginBottom: 6 }}>{b.label}</div>
                              <div style={{ fontSize: '1.8rem', fontWeight: 800, color: '#3d2008' }}>{b.guests}</div>
                              <div style={{ fontSize: '0.68rem', color: '#64748b', marginTop: 3 }}>
                                guests · {b.rsvps} responses
                              </div>
                              <div style={{ background: '#e8d5b7', height: 5, borderRadius: 4, marginTop: 10, overflow: 'hidden' }}>
                                <div
                                  style={{
                  ...
                  <div>
                      ...
                      <Download className="w-4 h-4" />
                        ...
                        {detailError && (
                          <div style={{ padding: '10px 14px', background: 'rgba(239,68,68,0.1)', borderRadius: 8, color: '#3d2008', fontSize: '0.82rem', marginBottom: 14 }}>
                            ⚠ {detailError}
                </div>
                        )}
                                ...
                                {filtered.map((item, idx) => {
                                  ...
                                  <tr key={item.rsvpId} style={S.tr}>
                                    <td style={{ ...S.td, color: '#475569' }}>{idx + 1}</td>
                                      <td style={{ ...S.td, fontWeight: 600, color: '#3d2008' }}>{item.name}</td>
                                    <td style={{ ...S.td, textAlign: 'center' }}>
...
// ── Theme-aware styles ────────────────────────────────────────────────────────
const S = {
  wrap:        { display: 'flex', flexDirection: 'column', height: '100%', background: 'linear-gradient(135deg, var(--cf-bg-base) 0%, var(--cf-bg-surface) 30%, var(--cf-bg-deep) 60%, var(--cf-bg-card) 100%)', color: 'var(--cf-text-primary)', fontFamily: 'Georgia, serif', borderRadius: 12, overflow: 'hidden' },
  header:      { background: 'var(--cf-bg-surface)', borderBottom: '1px solid var(--cf-border)', padding: '14px 20px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexShrink: 0 },
  closeBtn:    { background: 'transparent', border: '1px solid #e8d5a3', color: '#c9943a', padding: 8, borderRadius: 8, cursor: 'pointer', display: 'flex' },
  body:       { display: 'grid', gridTemplateColumns: '240px 1fr', gap: 0, flex: 1, overflow: 'hidden' },
  sidebar:     { background: 'var(--cf-bg-deep)', borderRight: '1px solid var(--cf-border)', padding: 16, overflowY: 'auto' },
  sectionLabel: { fontSize: '0.65rem', color: 'var(--cf-text-muted)', fontWeight: 700, letterSpacing: '0.1em', textTransform: 'uppercase', marginBottom: 10 },
  searchBox:   { display: 'flex', alignItems: 'center', gap: 8, background: 'var(--cf-bg-surface)', border: '1px solid var(--cf-border)', borderRadius: 8, padding: '7px 10px' },
  searchInput: { background: 'transparent', border: 'none', color: 'var(--cf-text-primary)', fontSize: '0.84rem', outline: 'none', flex: 1 },
  monthHeader: { fontSize: '0.68rem', color: 'var(--cf-accent)', fontWeight: 700, letterSpacing: '0.06em', marginBottom: 7, paddingBottom: 5, borderBottom: '1px solid var(--cf-border)', display: 'flex', justifyContent: 'space-between' },
  monthBadge:  { fontSize: '0.65rem', color: 'var(--cf-text-muted)', background: 'var(--cf-bg-surface)', padding: '1px 7px', borderRadius: 10 },
  eventCard:   { background: 'linear-gradient(145deg, #fffdf8 0%, var(--cf-bg-surface) 50%, var(--cf-bg-deep) 100%)', border: '1px solid var(--cf-border)', borderRadius: 7, padding: '9px 10px', cursor: 'pointer', marginBottom: 5, position: 'relative', transition: 'border-color 0.15s' },
  eventCardActive: { background: 'var(--cf-accent-glow)', borderColor: 'var(--cf-accent)' },
  eventCardTitle:  { fontSize: '0.8rem', fontWeight: 600, color: 'var(--cf-text-primary)', marginBottom: 2, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' },
  eventCardDate:   { fontSize: '0.68rem', color: 'var(--cf-text-muted)' },
  viewBadge:   { position: 'absolute', top: 7, right: 7, background: '#c2410c', color: '#fff', fontSize: '0.58rem', fontWeight: 700, padding: '2px 6px', borderRadius: 4 },
  tabs:        { display: 'flex', gap: 3, background: 'var(--cf-bg-deep)', border: '1px solid var(--cf-border)', borderRadius: 9, padding: 3, flexShrink: 0 },
  tab:         { flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 5, padding: '8px 12px', border: 'none', borderRadius: 6, cursor: 'pointer', fontSize: '0.82rem', fontWeight: 600, color: 'var(--cf-text-muted)', background: 'transparent', fontFamily: 'Georgia, serif' },
  tabActive:   { background: 'var(--cf-bg-surface)', color: 'var(--cf-text-primary)' },
  monthSelect: { background: 'var(--cf-bg-surface)', border: '1px solid var(--cf-border)', color: 'var(--cf-text-primary)', padding: '7px 12px', borderRadius: 8, fontSize: '0.88rem', cursor: 'pointer', outline: 'none', fontFamily: 'Georgia, serif' },
  card:        { background: 'linear-gradient(145deg, #fffdf8 0%, var(--cf-bg-surface) 50%, var(--cf-bg-deep) 100%)', borderRadius: 10, border: '1px solid var(--cf-border)', padding: 18 },
  cardTitle:   { fontSize: '0.88rem', fontWeight: 700, color: 'var(--cf-text-primary)', marginBottom: 4 },
  ...
  detailBtn:   { background: 'var(--cf-accent-glow)', border: '1px solid var(--cf-border-accent)', color: 'var(--cf-text-primary)', padding: '3px 9px', borderRadius: 6, cursor: 'pointer', fontSize: '0.72rem', fontWeight: 600 },
