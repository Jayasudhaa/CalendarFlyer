...
async function geocodeAddress(address) {
  ...
  // A bare `fetch()` here can hang indefinitely if the outbound request to
  // Google never gets a response (network egress blocked, DNS black-holed,
  // etc.) -- Node's fetch has no default timeout, so without this the whole
  // /api/discover/nearby request (and both "Happening near you" and
  // "Organizations near you" on the Explore page, which both depend on it)
  // would sit in "Finding..." forever with no way for the user to recover.
  // An AbortController with a bounded timeout turns that into the same
  // graceful "couldn't find that location" path as any other geocode
  // failure, rather than a silent infinite spinner.
  ...
  try {
    ...
    if (err.name === 'AbortError') {
      console.error(`[GEO] Geocode request timed out after ${GEOCODE_TIMEOUT_MS}ms for "${trimmed}"`);
    } else {
