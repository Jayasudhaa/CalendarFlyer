...
// The 10 language-association categories (see routes/discover.js's
// CATEGORY_TO_INTERESTS) each map to exactly one of these -- picking the
// "language" interest used to lump all ten together with no way to
// narrow further; this is what lets someone pick just Telugu + Kannada
// and see only those two.
const LANGUAGE_OPTIONS = [
  'telugu', 'kannada', 'tamil', 'malayalam', 'bengali',
  'odia', 'hindi', 'gujarati', 'marathi', 'punjabi',
];
...
/**
 * Sets display name, interests, languages, and/or cuisines. Each list is
 * filtered against its own fixed taxonomy rather than trusted as-is --
 * same reasoning as events.js's normalizeDiscoverability, a client
 * sending an unknown value shouldn't get to store it.
 */
async function updateIdentityProfile(identity_id, { display_name, interests, languages, cuisines } = {}) {
  const sets = [];
  const values = {};
  const names = {};
  ...
  if (languages !== undefined) {
    const clean = Array.isArray(languages)
      ? [...new Set(languages.filter((l) => LANGUAGE_OPTIONS.includes(l)))]
      : [];
    sets.push('#languages = :languages');
    names['#languages'] = 'languages';
    values[':languages'] = clean;
  }
  if (cuisines !== undefined) {
    const clean = Array.isArray(cuisines)
      ? [...new Set(cuisines.filter((c) => CUISINE_KEYS.includes(c)))]
      : [];
    sets.push('#cuisines = :cuisines');
    names['#cuisines'] = 'cuisines';
    values[':cuisines'] = clean;
  }
