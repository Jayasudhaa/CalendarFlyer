    ...
    <div style={S.page}>
      <style>{`
        @media (max-width: 640px) {
          .cf-rsvp-container { grid-template-columns: 1fr !important; padding: 12px !important; }
          .cf-rsvp-sidebar { max-height: 220px !important; }
          .cf-stat-grid-4 { grid-template-columns: repeat(2, 1fr) !important; }
          .cf-stat-grid-3 { grid-template-columns: repeat(2, 1fr) !important; }
          .cf-rsvp-tabs button { padding: 8px 6px !important; font-size: 0.75rem !important; }
          .cf-rsvp-table { font-size: 0.75rem !important; }
        }
      `}</style>
      {/* Header */}
      <div style={S.header}>
      ...
      <div className="cf-rsvp-container" style={S.container}>
        {/* ── Left: Event list ── */}
        <div style={S.sidebar}>
        ...
        <div style={{ flex: 1, minWidth: 0, display: 'flex', flexDirection: 'column', gap: 20 }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
              ...
              <div style={S.card}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 12, flexWrap: 'wrap' }}>
                  <span style={{ color: '#92400e', fontSize: '0.8rem', fontWeight: 700, letterSpacing: '0.08em', textTransform: 'uppercase' }}>
                    Select Month
                  </span>
                  <select
                    value={selectedMonth}
                  ...
                  {monthLoading && (
                    <span style={{ color: '#9a7a55', fontSize: '0.8rem' }}>⏳ Loading...</span>
            )}
      </div>
        </div>
              {/* Monthly summary cards */}
              {!monthLoading && monthlyData.length > 0 && (
                <div className="cf-stat-grid-4" style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 14 }}>
                  {[
              ...
              <div style={S.card}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 18 }}>
                  ...
                  <div style={{ fontSize: '0.75rem', color: '#9a7a55' }}>
                    {availableMonths.find(m => m.key === selectedMonth)?.label}
    </div>
                </div>

                {monthLoading ? (
                  <div style={{ textAlign: 'center', padding: '40px 0', color: '#9a7a55' }}>
                  ...
                  <div style={{ overflowX: 'auto' }}>
                    <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                          ...
                          .map(({ event, rsvp }) => {
                            ...
                            return (
                              <tr
                                ...
                                onClick={() => handleEventSelect(event)}
                              >
                                <td style={{ ...S.td, color: '#92400e', whiteSpace: 'nowrap' }}>{dateStr}</td>
                                <td style={{ ...S.td, fontWeight: 600, color: '#3d2008', maxWidth: 220 }}>
                      ...
                      {/* Totals row */}
                      <tfoot>
                        <tr style={{ borderTop: '2px solid #e8d5b7', background: '#fdf8f0' }}>
                          <td style={{ ...S.td, color: '#9a7a55' }} />
          ...
          {activeTab === 'detail' && (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
              {!selectedEvent ? (
                <div style={{ ...S.card, textAlign: 'center', padding: '60px 0', color: '#9a7a55' }}>
                ...
                <>
                  ...
                  <div style={{ ...S.card, padding: '16px 20px' }}>
                    <div style={{ color: '#3d2008', fontWeight: 700, fontSize: '1rem' }}>{selectedEvent.title}</div>
                    <div style={{ color: '#9a7a55', fontSize: '0.8rem', marginTop: 4 }}>
                  ...
                  {/* Stat cards */}
                  {detailLoading ? (
                    <div style={{ textAlign: 'center', padding: 40, color: '#9a7a55' }}>⏳ Loading...</div>
                  ) : (
                    <>
                      <div className="cf-stat-grid-4" style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 14 }}>
                        {[
                      ...
                      <div style={S.card}>
                        ...
                        <div className="cf-stat-grid-3" style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 14, marginTop: 16 }}>
                          {[
                            ...
                            <div key={b.label} style={{ background: '#fdf8f0', borderRadius: 8, padding: 16 }}>
                              <div style={{ fontSize: '0.85rem', fontWeight: 600, color: b.color, marginBottom: 8 }}>{b.label}</div>
                              <div style={{ fontSize: '2rem', fontWeight: 800, color: '#3d2008' }}>{b.guests}</div>
                              <div style={{ fontSize: '0.72rem', color: '#9a7a55', marginTop: 4 }}>guests · {b.rsvps} responses</div>
                              <div style={{ background: '#e8d5b7', height: 6, borderRadius: 4, marginTop: 10, overflow: 'hidden' }}>
                    <div style={{
            ...
            <div>
                            ...
                            <div style={{ fontSize: '0.75rem', color: '#9a7a55', marginTop: 2 }}>{filtered.length} entries</div>
              </div>
          ...
          {filtered.length === 0 ? (
                          <div style={{ textAlign: 'center', padding: '48px 0', color: '#9a7a55' }}>
                    ...
                    {filtered.map((item, idx) => (
                            ...
                            <span style={{
                              ...
                              {item.attending === 'yes' ? '✅ Yes' : item.attending === 'no' ? '❌ No' : '🤔 Maybe'}
                                    ...
                                    <td style={{ ...S.td, color: '#9a7a55' }}>{item.phone || '—'}</td>
                                    <td style={{ ...S.td, color: '#9a7a55', whiteSpace: 'nowrap' }}>
...
// ── Styles ────────────────────────────────────────────────────────────────────
const S = {
  page:       { minHeight: '100vh', background: '#fdf8f0', color: '#3d2008', fontFamily: 'system-ui, sans-serif' },
  header:     { background: '#fffdf7', borderBottom: '1px solid #e8d5b7', padding: '20px 28px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' },
  title:      { fontSize: '1.6rem', fontWeight: 800, color: '#fff', margin: 0, display: 'flex', alignItems: 'center' },
  subtitle:   { fontSize: '0.85rem', color: '#92400e', margin: '4px 0 0' },
  closeBtn:   { background: 'transparent', border: '1px solid #e8d5b7', color: '#92400e', padding: 10, borderRadius: 8, cursor: 'pointer', display: 'flex', alignItems: 'center' },
  container:  { display: 'grid', gridTemplateColumns: 'clamp(200px, 28%, 280px) 1fr', gap: 24, padding: '24px 28px', maxWidth: 1400, margin: '0 auto' },
  sidebar:    { background: '#fffdf7', borderRadius: 12, border: '1px solid #e8d5b7', padding: 18, height: 'fit-content', maxHeight: 'calc(100vh - 120px)', overflowY: 'auto' },
  sectionLabel: { fontSize: '0.68rem', color: '#9a7a55', fontWeight: 700, letterSpacing: '0.1em', textTransform: 'uppercase', marginBottom: 12 },
  searchBox:  { display: 'flex', alignItems: 'center', gap: 8, background: '#fdf8f0', border: '1px solid #e8d5b7', borderRadius: 8, padding: '8px 12px' },
  searchInput: { background: 'transparent', border: 'none', color: '#3d2008', fontSize: '0.88rem', outline: 'none', flex: 1 },
  eventList:  { marginTop: 14 },
  monthHeader: { fontSize: '0.72rem', color: '#92400e', fontWeight: 700, letterSpacing: '0.06em', marginBottom: 8, paddingBottom: 6, borderBottom: '1px solid #e8d5b7', display: 'flex', justifyContent: 'space-between', alignItems: 'center' },
  monthBadge: { fontSize: '0.68rem', color: '#9a7a55', background: '#fdf8f0', padding: '2px 8px', borderRadius: 10 },
  eventCard:  { background: '#fdf8f0', border: '1px solid #e8d5b7', borderRadius: 8, padding: '10px 12px', cursor: 'pointer', marginBottom: 6, position: 'relative', transition: 'border-color 0.15s' },
  eventCardActive: { background: '#1e40af22', borderColor: '#3b82f6' },
  eventCardTitle: { fontSize: '0.85rem', fontWeight: 600, color: '#3d2008', marginBottom: 3, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' },
  eventCardDate:  { fontSize: '0.72rem', color: '#9a7a55' },
  ...
  tabs:       { display: 'flex', gap: 4, background: '#fffdf7', border: '1px solid #e8d5b7', borderRadius: 10, padding: 4 },
  tab:        { flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6, padding: '10px 16px', border: 'none', borderRadius: 7, cursor: 'pointer', fontSize: '0.85rem', fontWeight: 600, color: '#9a7a55', background: 'transparent', transition: 'all 0.15s' },
  tabActive:  { background: '#fdf8f0', color: '#3d2008', boxShadow: '0 1px 4px rgba(0,0,0,0.3)' },
  monthSelect: { background: '#fdf8f0', border: '1px solid #e8d5b7', color: '#3d2008', padding: '8px 14px', borderRadius: 8, fontSize: '0.9rem', cursor: 'pointer', outline: 'none' },
  card:       { background: '#fffdf7', borderRadius: 12, border: '1px solid #e8d5b7', padding: 22 },
  cardTitle:  { fontSize: '0.9rem', fontWeight: 700, color: '#3d2008', marginBottom: 4 },
  statCard:   { borderRadius: 12, padding: '20px 16px', textAlign: 'center', boxShadow: '0 4px 16px rgba(0,0,0,0.2)' },
  th:         { padding: '10px 14px', textAlign: 'left', color: '#9a7a55', fontWeight: 600, fontSize: '0.72rem', textTransform: 'uppercase', letterSpacing: '0.05em' },
  tr:         { borderBottom: '1px solid #e8d5b7', transition: 'background 0.15s' },
  td:         { padding: '12px 14px', fontSize: '0.84rem', color: '#92400e' },
  ...
  iconBtn:    { background: '#fdf8f0', border: '1px solid #e8d5b7', color: '#92400e', padding: 9, borderRadius: 8, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' },
