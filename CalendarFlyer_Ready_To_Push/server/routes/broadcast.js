...
router.post('/whatsapp-template', async (req, res) => {
  ...
  const testNumber = process.env.TEST_RECIPIENT_NUMBER;
  const recipientEnv = process.env.RECIPIENT_NUMBERS || '';
  const recipients   = testMode
    ? [testNumber]
    : recipientEnv.split(',').map(n => n.trim()).filter(Boolean);
  if (!token)   return res.status(500).json({ error: 'WHATSAPP_TOKEN not set in App Runner env vars' });
  if (!phoneId) return res.status(500).json({ error: 'WHATSAPP_PHONE_ID not set in App Runner env vars' });
  if (!recipients.length) return res.status(400).json({ error: 'No recipients. Set RECIPIENT_NUMBERS in App Runner env vars.' });
  if (!template_name)     return res.status(400).json({ error: 'template_name is required' });
  console.log(`[BROADCAST] 📋 Template: ${template_name} | Recipients: ${recipients.length} | TestMode: ${testMode}`);
  ...
  for (const to of recipients) {
    try {
      ...
      if (!response.ok || data.error) {
        console.error(`[BROADCAST] ✗ Template to ${to}:`, data.error?.message);
        errors.push({ to, error: data.error?.message || 'Unknown error' });
      } else {
        console.log(`[BROADCAST] ✓ Template sent to ${to}`);
        sent++;
      }
    } catch (err) {
      console.error(`[BROADCAST] ✗ Template to ${to}:`, err.message);
      errors.push({ to, error: err.message });
    }
  }
  if (sent === 0) {
    return res.status(500).json({ error: errors[0]?.error || 'All sends failed', errors });
  }
  return res.json({ success: true, sent, total: recipients.length, errors });
});
