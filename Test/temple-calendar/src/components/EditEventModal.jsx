  ...
  useEffect(() => {
    if (event) {
      setForm({
        ...
        discoverability: event.discoverability === 'radar' ? 'radar' : 'org_only'
      });
    }
  }, [event]);
    ...
    <div style={{ position:"fixed", inset:0, background:"rgba(0,0,0,0.55)", display:"flex", alignItems:"center", justifyContent:"center", zIndex:9999, padding:16, backdropFilter:"blur(4px)" }}>
      <div style={{ background:"linear-gradient(160deg,#fdf6e9,#fef9f0)", borderRadius:20, boxShadow:"0 24px 60px rgba(0,0,0,0.3)", width:"100%", maxWidth:520, border:"1px solid #d4af37", animation:'slideIn 0.25s ease-out' }}>
        ...
        <div style={{ background:"linear-gradient(135deg,#b83a0a,#8a2c08)", color:"#fff", padding:"18px 20px", borderRadius:"20px 20px 0 0", display:"flex", alignItems:"center", justifyContent:"space-between" }}>
          <h2 style={{ fontSize:"1.15rem", fontWeight:700, fontFamily:"'Playfair Display', Georgia, serif" }}>Edit Event</h2>
        ...
        <form onSubmit={handleSubmit} style={{ padding:"20px 24px 24px" }}>
          ...
          {/* Discoverability — Community Radar opt-in. Kept off the org's own
              calendar behavior entirely: this only controls whether the
              event ALSO appears in the cross-org Radar feed. */}
          <div style={{ margin: "10px 0" }}>
            <label style={{ display:"block", fontSize:"0.82rem", fontWeight:600, color:"#6b2d0a", marginBottom:4 }}>Discoverability</label>
            <div style={{ border:"1px solid #d4af37", borderRadius:8, background:"#fffdf7", padding:"10px 12px", display:"flex", alignItems:"flex-start", gap:10 }}>
              <input
                type="checkbox"
                id="edit-event-radar"
                checked={form.discoverability === 'radar'}
                onChange={(e) => setForm(prev => ({ ...prev, discoverability: e.target.checked ? 'radar' : 'org_only' }))}
                style={{ marginTop: 3, width: 16, height: 16, accentColor: '#9a3412', cursor: 'pointer', flexShrink: 0 }}
              />
              <label htmlFor="edit-event-radar" style={{ cursor: 'pointer' }}>
                <div style={{ fontSize: '0.85rem', fontWeight: 600, color: '#3d2008' }}>Show on Explore</div>
                <div style={{ fontSize: '0.75rem', color: '#9a7a55', marginTop: 2, lineHeight: 1.4 }}>
                  Also surfaced to people outside your organization. Off = your public calendar only.
                </div>
              </label>
            </div>
          </div>
