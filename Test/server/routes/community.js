/**
 * server/routes/community.js — email verification for devotees (see
...
// as the fallback for a request with no email in the body yet).
...
router.post('/verify-code', async (req, res) => {
  try {
    ...
    const member = await verifyOtp(req.org.org_id, email, code);
    const token = issueCommunityToken(member);
    res.json({ success: true, token, member: { member_id: member.member_id, email: member.email } });
  } catch (err) {
...
// -- no email addresses or names go back to the dashboard, matching the "no
