...
//
// 'live' on the photo itself only means "an admin approved it" -- it says
// nothing about whether the ALBUM it belongs to is still private, hidden,
// or a draft the admin hasn't published yet (see photoAlbums.js's header
// comment: draft is "visible to admins only, never shown on the public
// site regardless of visibility"). This route used to skip that check
// entirely and hand back every live photo for the event, so a devotee
// could see photos from an album the admin never intended to publish.
// A photo with no album_id (pre-albums-feature data) has no such setting
// to enforce, so it stays visible, matching this endpoint's behavior
// before albums existed.
router.get('/event/:eventId', authenticateCommunityToken, async (req, res) => {
  try {
    ...
    const [photos, albums] = await Promise.all([
      listLivePhotosForEvent(req.params.eventId, { limit: 100 }),
      listAlbumsForEvent(req.params.eventId),
    ]);
    const albumById = new Map(albums.map((a) => [a.album_id, a]));

    const visiblePhotos = photos.filter((p) => {
      // No album to vouch for this photo's visibility -- either it never
      // had one, or its album was deleted since -- means hide it. The
      // first cut of this fix defaulted a missing album to "visible",
      // which meant deleting a private album (rather than just leaving it
      // private) made its photos public again, the opposite of what
      // deleting it should do.
      if (!p.album_id) return false;
      const album = albumById.get(p.album_id);
      if (!album) return false;
      if (album.publishing_status === 'draft') return false;
      if (album.visibility === 'private' || album.visibility === 'hidden') return false;
      return true;
    });
