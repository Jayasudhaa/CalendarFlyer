...
const {
  INTEREST_OPTIONS, sendIdentityOtp, verifyIdentityOtp, getIdentity, updateIdentityProfile,
...
router.get('/interest-options', (req, res) => {
  res.json({ interests: INTEREST_OPTIONS });
});
...
router.patch('/me', authenticateIdentityToken, async (req, res) => {
  try {
    const { display_name, interests } = req.body || {};
    const updated = await updateIdentityProfile(req.identityUser.identity_id, { display_name, interests });
    res.json({
      success: true,
