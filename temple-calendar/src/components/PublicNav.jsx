 ...
 * org page (Calendar · Announcements · Glimpses · Photos), rendered once
 * from PublicOrgLayout (App.jsx) above the routed page, the same way that
 * layout already mounts WebChatWidget once for every visitor page.
 *
 ...
 * "New event glimpse" banner linking to the Glimpses tab. Older than
...
import { Calendar, Film, Image, Megaphone } from 'lucide-react';
...
const NAV_ITEMS = [
  ...
  { to: '/live', label: 'Glimpses', icon: Film },
    ...
    <>
      <nav style={navStyle}>
        {NAV_ITEMS.map(({ to, label, icon: Icon }) => (
          ...
          >
            <Icon size={17} />
            <span className="cf-public-nav-label">{label}</span>
          </NavLink>
        ))}
      </nav>

      {showBanner && (
        <a href={`/live${window.location.search}`} style={bannerStyle('#1e3a8a', '#1d4ed8')}>
