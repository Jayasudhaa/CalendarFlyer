...
export default function AdminAssistant({ events = [], onAction }) {
  ...
  const sendMessage = async (text) => {
    ...
    try {
      ...
      // /api/chat/admin-assistant requires a logged-in admin (authenticateToken
      // on the server) but this fetch never sent the auth token, so the server
      // always rejected it with 401 { error: 'Access token required' } — a
      // response with no `reply` field, which is why every single message
      // (not just this one) fell through to the generic fallback text below.
      ...
      // Backend returns { reply, action } directly. This fallback should be
      // rare now (server/routes/chat.js's callClaude always returns a real
      // reply or an honest "didn't get a clear answer" message) — it only
      // hits on a genuinely malformed/empty response, so it shouldn't claim
      // to have understood the question either.
      const reply  = data.reply  || "Sorry, something went wrong on my end — please try asking again.";
      ...
      if (action && onAction) {
        const actionMap = {
          ...
          // Opens Settings on its Social Media tab (Facebook/Instagram/
          // WhatsApp connect), for "connect my Facebook/Instagram page"
          // type requests — see the system prompt in server/routes/chat.js.
          openSocialSettings: 'connectSocial',
          openHelp:          'help',
        };
  ...
  const S = {
    // 10050 — above Broadcast/Flyer Studio's full-screen overlays (both
    // zIndex:9999, Flyer's editing chrome up to 10001), which used to sit
    // on top of this bubble and hide it entirely whenever either was open,
    // even though the component itself stayed mounted underneath them.
    wrapper: { position: 'fixed', bottom: 24, right: 24, zIndex: 10050, display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: 10, fontFamily: "'DM Sans', sans-serif" },
    ...
    pill: { padding: '4px 10px', borderRadius: 20, border: `1px solid ${B}`, background: C, color: G, fontSize: '0.7rem', cursor: 'pointer', fontWeight: 600, fontFamily: "'DM Sans', sans-serif", transition: 'all 0.12s' },
    inputRow: { display: 'flex', gap: 7, padding: '9px 12px', borderTop: `1px solid ${B}`, background: C, flexShrink: 0 },
    input: { flex: 1, padding: '8px 11px', borderRadius: 20, border: `1px solid ${B}`, background: D, color: W, fontSize: '0.82rem', outline: 'none', fontFamily: "'DM Sans', sans-serif" },
