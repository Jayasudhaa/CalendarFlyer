...
export default function RSVPAnalyticsPage({ events = [], onClose }) {
  ...
  // ── AI-powered insights (Organization Pro+) — see server/routes/
  // analyticsInsights.js. Generated on demand (not auto-fetched) so a
  // Free/Plus org doesn't burn a 403 round-trip just from opening the tab.
  const [aiInsights, setAiInsights] = useState('');
  const [aiInsightsLoading, setAiInsightsLoading] = useState(false);
  const [aiInsightsError, setAiInsightsError] = useState('');
  const [aiInsightsUpgradeNeeded, setAiInsightsUpgradeNeeded] = useState(false);
  ...
  // Per-event slice of the same real data, shaped for the AI insights
  // request below -- same {yes,no,maybe} counting as `overview` above,
  // just not summed across the whole month.
  const eventBreakdownForAI = React.useMemo(() => {
    return monthlyDataFiltered.map(({ event, rsvp }) => {
      let yes = 0, no = 0, maybe = 0, guests = 0;
      (rsvp.items || []).forEach((item) => {
        const a = getAttendance(item);
        guests += item.count || 0;
        if (a === 'yes') yes++;
      ...
      return { title: event.title, date: event.date, rsvps: (rsvp.items || []).length, yes, no, maybe, guests };
    });
  }, [monthlyDataFiltered]);
  ...
  const generateAiInsights = async () => {
    ...
    setAiInsightsError('');
    setAiInsightsUpgradeNeeded(false);
    setAiInsights('');
    try {
      const res = await fetch(`${API}/api/organizations/analytics/insights`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', ...authHeader() },
        body: JSON.stringify({ overview, weeklyRegistrations, eventBreakdown: eventBreakdownForAI, monthLabel: selectedMonth }),
      });
      const data = await res.json().catch(() => ({}));
      if (res.status === 403) {
        setAiInsightsUpgradeNeeded(true);
        setAiInsightsError(data.error || 'AI-powered analytics is available on the Organization Pro plan — upgrade to unlock it.');
        return;
      }
      if (!res.ok) throw new Error(data.error || 'Failed to generate insights');
      setAiInsights(data.insights || '');
    } catch (err) {
      setAiInsightsError(err.message || 'Failed to generate insights');
    } finally {
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
            ...
            <div style={S.card}>
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12, flexWrap: 'wrap' }}>
                <div>
                  <div style={S.cardTitle}>✨ AI-powered insights</div>
                  <div style={S.cardSub}>A plain-English read on this month's numbers, generated by Claude from the data above.</div>
                </div>
                <button onClick={generateAiInsights} disabled={aiInsightsLoading} style={{ ...S.exportBtn, opacity: aiInsightsLoading ? 0.6 : 1, cursor: aiInsightsLoading ? 'default' : 'pointer' }}>
                  {aiInsightsLoading ? 'Generating…' : aiInsights ? 'Regenerate' : 'Generate insights'}
                </button>
              </div>
              {aiInsightsError && (
                <div style={{ marginTop: 12, fontSize: '0.85rem', color: aiInsightsUpgradeNeeded ? 'var(--cf-text-muted)' : '#dc2626' }}>
                  {aiInsightsError}
                  {aiInsightsUpgradeNeeded && (
                    <> <a href="/profile?tab=subscription" style={{ color: 'var(--cf-accent)', fontWeight: 700 }}>Upgrade to Organization Pro →</a></>
                  )}
                </div>
              )}
              {aiInsights && !aiInsightsError && (
                <>
                  <p style={{ marginTop: 14, fontSize: '0.92rem', lineHeight: 1.6, color: 'var(--cf-text-primary)', whiteSpace: 'pre-wrap' }}>{aiInsights}</p>
                  <div style={{ marginTop: 8, fontSize: '0.72rem', color: 'var(--cf-text-muted)' }}>AI-generated from your own RSVP data — double-check anything important.</div>
                </>
              )}
            </div>
