...
// organization is the templeConfig object (useTempleConfig) — its org_id is
// sent to the backend below so /api/chat/temple-bot resolves the right
// org's persona/contact info instead of falling back to a generic one, and
// its temple_name drives the welcome message instead of a hardcoded sample.
export default function WebChatWidget({ events = [], organization = null }) {
  const orgName = organization?.temple_name || 'Sample Temple Name';
  const welcomeText = `🙏 Namaste! I am the ${orgName} assistant. Ask me about events, poojas, timings, or anything about our temple!`;
  ...
  // useTempleConfig resolves the real org name asynchronously, so this
  // widget usually mounts before it's known and the welcome bubble above
  // was built from the still-default `organization`. Once the real name
  // arrives, refresh just that bubble — but only while it's still the
  // untouched welcome message, so an in-progress conversation is never
  // clobbered by a late-arriving org name.
  useEffect(() => {
    setMessages(prev =>
      prev.length === 1 && prev[0].id === 'welcome' && prev[0].text !== welcomeText
  ...
  const sendMessage = async (text) => {
    ...
    try {
      const res = await fetch('/api/chat/temple-bot', {
        ...
        // org_id lets the backend resolve this org's persona/contact info
        // when req.org isn't already set from subdomain/tenant middleware
        // (e.g. local dev, or a custom domain not yet wired to it) — see
        // routes/chat.js's /temple-bot handler. Without this the bot always
        // answered as a generic, org-less assistant.
        body: JSON.stringify({ message: msg, upcomingEvents: upcomingText, org_id: organization?.org_id || null }),
      });
  ...
  return (
    <div style={{ position:'fixed', bottom:24, right:24, zIndex:9100, display:'flex', flexDirection:'column', alignItems:'flex-end', gap:10, fontFamily:"'DM Sans', sans-serif" }}>
      <style>{`
        ...
        <div style={{
          ...
          <div style={{ background:'linear-gradient(135deg,#8B4513,#c2410c)', padding:'13px 15px', display:'flex', alignItems:'center', gap:10, flexShrink:0 }}>
            ...
            <button onClick={openWA} className="wc-wa" title="Open WhatsApp"
              style={{ background:'#25d366', border:'none', borderRadius:8, padding:'5px 8px', cursor:'pointer', display:'flex', alignItems:'center', gap:4, color:'#fff', fontSize:'0.68rem', fontWeight:700, fontFamily:"'DM Sans', sans-serif", transition:'all 0.15s' }}>
              💬 WA
            </button>
            ...
            <div style={{ padding:'6px 10px 4px', display:'flex', flexWrap:'wrap', gap:5, borderTop:'1px solid #2a1208', flexShrink:0 }}>
              {SUGGESTED_QUESTIONS.map((q,i) => (
                <button key={i} onClick={() => sendMessage(q)} className="wc-pill"
                  style={{ padding:'4px 10px', borderRadius:18, border:'1px solid #3a2008', background:'#1e1208', color:'#c9943a', fontSize:'0.68rem', cursor:'pointer', fontWeight:600, fontFamily:"'DM Sans', sans-serif", transition:'all 0.12s' }}>
                  {q}
                </button>
              ))}
            </div>
          ...
          <div style={{ display:'flex', gap:7, padding:'9px 10px', borderTop:'1px solid #2a1208', background:'#1e1208', flexShrink:0 }}>
            <input
              ...
              disabled={loading}
              maxLength={300}
              style={{ flex:1, padding:'8px 11px', borderRadius:20, border:'1px solid #3a2008', background:'#160b02', color:'#f0e0b8', fontSize:'0.8rem', fontFamily:"'DM Sans', sans-serif", transition:'border-color 0.15s' }}
            />
