...
const PLATFORMS = [
  ...
  },
  {
    ...
    icon: '📧',
    color: '#6366f1',
    description: 'Send to your mailing list (set under Settings)',
  },
];
...
const TEMPLATES = [
  ...
  // Shared by the WhatsApp/Facebook "open with message" buttons and the
  // caption builders below — the event's public RSVP page (or the general
  // calendar if there's no date yet to build a slug from).
  function buildEventUrl(ev) {
    const baseUrl = 'https://calendarflyapp.com';
    if (!ev?.date) return `${baseUrl}/calendar`;
    const titleSlug = (ev.title || 'event').toLowerCase().trim()
      .replace(/[^\w\s-]/g, '').replace(/\s+/g, '-').replace(/^-+|-+$/g, '');
    return `${baseUrl}/rsvp/${ev.date}-${titleSlug}`;
  }
...
function buildTemplate(ev, type) {
  ...
  const date     = ev?.date     || '';
  const time     = ev?.time     || '';
  const location = ev?.location || 'Sri Venkateswara Swamy Temple of Colorado';
  const dateSlug  = date;
...
export default function BroadcastModal({ onClose, fabricRef, event }) {
  ...
  // Dev mode / Prod mode — which credential set the "📣 API Broadcast"
  // button below will actually hit (server/routes/broadcast.js). Fetched
  // once so a test send can't be mistaken for a real one, or vice versa.
  const [socialMode, setSocialMode] = useState(null);
  React.useEffect(() => {
    fetch('/api/broadcast/mode', { headers: { Authorization: `Bearer ${localStorage.getItem('cf_token')}` } })
      .then(r => r.ok ? r.json() : null)
      .then(data => data && setSocialMode(data))
  ...
  // ── "Open [platform] with message" — no API needed ───────────────────────
  // Same idea as the WhatsApp button below (wa.me), extended to Facebook and
  // Instagram. Neither platform offers WhatsApp's trick directly, though:
  ...
  //     link can do is open the dialog pointed at the event's RSVP page.
  ...
  // the OS-level share sheet, and Instagram/Facebook (and anything else
  // installed) show up in it exactly like they do from the Photos app. Tried
  // first everywhere; if it's unavailable (desktop, unsupported browser) or
  // the user backs out, each platform falls back to the best next thing.
  async function shareViaWebShareOrFallback(fallbackFn) {
    try {
      if (navigator.share) {
        const imageBase64 = getFlyerBase64();
        const shareData = { text: caption, title: event?.title || 'Temple Event' };
        if (imageBase64) {
      ...
      console.warn('[BroadcastModal] Web Share failed, falling back:', err);
    }
    fallbackFn();
  }
    ...
    shareViaWebShareOrFallback(async () => {
      try {
        ...
        if (imageBase64) {
          ...
          a.href = imageBase64;
          a.download = `flyer-${(event?.title || 'temple-event').toLowerCase().replace(/[^\w-]+/g, '-')}.png`;
          document.body.appendChild(a);
          a.click();
          document.body.removeChild(a);
        }
        if (navigator.clipboard?.writeText) {
          try { await navigator.clipboard.writeText(caption); } catch (e) { /* clipboard permission denied — download still happened */ }
        }
        ...
        alert('Could not prepare the flyer — try again, or save it from the canvas toolbar instead.');
      }
    });
  }
  ...
  const S = {
    ...
    broadcastBtn: (disabled) => ({
      ...
      background: disabled
        ? '#1f2937'
        : 'linear-gradient(180deg, rgba(255,255,255,0.16) 0%, rgba(255,255,255,0) 55%), linear-gradient(135deg, #57534e 0%, #000000 100%)',
        ...
        <div style={{ padding: '18px 22px 22px' }}>
          ...
          <div style={{ marginBottom: 18 }}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
              ...
              {/* Dev/Prod badge for the "📣 API Broadcast" button below — see
                  socialMode() in server/routes/broadcast.js. Silent until it
                  loads so it never flashes the wrong state. */}
              {socialMode && (
                <div title={socialMode.mode === 'dev' ? 'API Broadcast will use DEV credentials — safe to test' : 'API Broadcast will use PROD credentials — this goes out for real'} style={{
                  fontSize: '0.64rem', fontWeight: '800', letterSpacing: '0.06em', textTransform: 'uppercase',
                  padding: '3px 9px', borderRadius: 20, marginBottom: 8,
                  color: socialMode.mode === 'dev' ? '#fbbf24' : '#34d399',
                  background: socialMode.mode === 'dev' ? 'rgba(251,191,36,0.1)' : 'rgba(52,211,153,0.1)',
                  border: `1px solid ${socialMode.mode === 'dev' ? 'rgba(251,191,36,0.25)' : 'rgba(52,211,153,0.25)'}`,
                }}>
                  {socialMode.mode === 'dev' ? '🧪 Dev Mode' : '🚀 Prod Mode'}
                </div>
              )}
            </div>
              {PLATFORMS.map(p => {
          ...
          {/* ── WhatsApp Broadcast List — quick share, no account connection ── */}
          <div style={{
            marginTop: 16,
            padding: '12px 14px',
            ...
            <div style={{ color: '#4b5563', fontSize: '0.68rem', fontWeight: '700', letterSpacing: '0.08em', textTransform: 'uppercase', marginBottom: 6 }}>
              💬 WhatsApp Broadcast List — Quick Share
            </div>
          ...
          {/* ── Facebook — quick share, no account connection ── */}
          <div style={{
            marginTop: 10,
            padding: '12px 14px',
            ...
            <div style={{ color: '#6b7280', fontSize: '0.72rem', marginBottom: 10, lineHeight: 1.5 }}>
              On a phone, shares the flyer image straight into Facebook via your share sheet. Elsewhere, opens Facebook's share dialog for the event page — Facebook doesn't let any app pre-fill caption text, only a link.
            </div>
            <button
          ...
          {/* ── Instagram — quick share, no account connection ── */}
          <div style={{
            marginTop: 10,
            padding: '12px 14px',
            ...
            <div style={{ color: '#6b7280', fontSize: '0.72rem', marginBottom: 10, lineHeight: 1.5 }}>
              On a phone, shares the flyer image straight into Instagram via your share sheet. Elsewhere, Instagram allows no direct share link at all — this downloads the flyer and copies the caption so you can paste them in yourself.
            </div>
            <button
