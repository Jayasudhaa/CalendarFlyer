...
import { getRsvpUrl, getAdminUrl, getQrUrl, getPhotoAlbumUrl, getPhotoAlbumQrUrl } from '../utils/rsvpUrl';
...
export default function RSVPShare({ event, compact = false }) {
  ...
  const photoQrUrl = getPhotoAlbumQrUrl(event, 200);
  ...
  function photoWhatsappShare() {
    const msg = `📸 Share your photos from *${event.title}*!\n\nVerify your phone number and add your pictures to the live album:\n${photoUrl}`;
    window.open(`https://wa.me/?text=${encodeURIComponent(msg)}`, '_blank');
  };
  ...
  return (
    <div style={{
      ...
      padding: compact ? '12px' : '16px', fontFamily: "'DM Sans', sans-serif",
    }}>
      ...
      {/* Photo Album link */}
      <div style={{ borderTop: '1px solid #1f2937', paddingTop: 10, marginTop: 10, marginBottom: 4 }}>
        <div style={{ color: '#9ca3af', fontSize: '0.65rem', textTransform: 'uppercase', letterSpacing: '0.08em', marginBottom: 10 }}>
          📸 Photo Album Link
        </div>
        ...
        <div style={{ display: 'flex', gap: 6, marginBottom: 10 }}>
          ...
          <button onClick={() => copy(photoUrl, setCopiedPhoto)}
            style={{ padding: '7px 12px', background: copiedPhoto ? '#16a34a' : '#1f2937',
              border: '1px solid #374151', color: 'white', borderRadius: 6, cursor: 'pointer',
              fontSize: '0.72rem', fontWeight: '600', whiteSpace: 'nowrap', transition: 'background 0.2s' }}>
            {copiedPhoto ? '✓ Copied!' : '📋 Copy'}
          </button>
        </div>

        <div style={{ display: 'flex', gap: 6, marginBottom: 10, flexWrap: 'wrap' }}>
          <button onClick={photoWhatsappShare}
            style={{ flex: 1, padding: '8px', background: '#166534', border: 'none',
              color: 'white', borderRadius: 6, cursor: 'pointer', fontSize: '0.75rem', fontWeight: '600' }}>
            💬 WhatsApp
          </button>
          <button onClick={() => setShowPhotoQr(q => !q)}
            style={{ flex: 1, padding: '8px', background: showPhotoQr ? '#1d4ed8' : '#1f2937',
              border: '1px solid #374151', color: 'white', borderRadius: 6, cursor: 'pointer', fontSize: '0.75rem' }}>
            {showPhotoQr ? '✕ Hide QR' : '⬛ QR Code'}
          </button>
        ...
        {showPhotoQr && (
          <div style={{ textAlign: 'center', padding: '12px 0',
            background: '#0d1117', borderRadius: 8, border: '1px solid #1f2937' }}>
            <img src={photoQrUrl} alt="Photo Album QR Code" width={160} height={160}
              style={{ borderRadius: 8, display: 'block', margin: '0 auto 8px' }} />
            <div style={{ fontSize: '0.65rem', color: '#6b7280' }}>Scan to share photos</div>
            <a href={photoQrUrl} download={`photo_album_qr_${event.title?.replace(/\s+/g,'_')}.png`}
              style={{ fontSize: '0.68rem', color: '#3b82f6', textDecoration: 'none', display: 'block', marginTop: 4 }}>
              ⬇ Download QR
            </a>
          </div>
