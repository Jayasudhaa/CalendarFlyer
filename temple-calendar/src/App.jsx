...
function AdminCalendar() {
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#0f172a', display: 'flex', flexDirection: 'column', fontFamily: 'Georgia, serif' }}>
      ...
      {showRSVPAnalytics && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.75)', zIndex: 9999, overflowY: 'auto' }} onClick={() => closeModal(setShowRSVPAnalytics)}>
          <div onClick={e => e.stopPropagation()}>
            <RSVPAnalyticsPage events={events} />
          </div>
        </div>
      )}
    </div>
