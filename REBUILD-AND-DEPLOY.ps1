...
# Builds and deploys production only when explicitly invoked with its verified service ARN.
...
Write-Host "[OK] dist and vite cache cleared" -ForegroundColor Green
...
Write-Host "[OK] Built: $bundle" -ForegroundColor Green
...
if ($bundleContent -match "jyuxa8xvk6") {
    Write-Host "[FAIL] OLD URL still in bundle! Source files not updated." -ForegroundColor Red
    exit 1
} else {
    Write-Host "[OK] Bundle is clean" -ForegroundColor Green
}
...
if ($LASTEXITCODE -ge 8) { Write-Host "[FAIL] Copy to dist-frontend failed!" -ForegroundColor Red; exit 1 }
Write-Host "[OK] Copied to server\dist-frontend (old bundles removed)" -ForegroundColor Green
...
if ($LASTEXITCODE -ne 0) { Write-Host "[FAIL] Docker build failed!" -ForegroundColor Red; exit 1 }
...
if (-not $serviceArn -or $serviceArn -eq "None") {
    Write-Host "[FAIL] Could not find an App Runner service in $region -- check the AWS CLI is logged in to the right account." -ForegroundColor Red
    exit 1
}
Write-Host "Service: $serviceArn"
aws apprunner start-deployment --service-arn $serviceArn --region $region | Out-Null
if ($LASTEXITCODE -ne 0) { Write-Host "[FAIL] Could not start the App Runner deployment!" -ForegroundColor Red; exit 1 }
...
while ($true) {
    ...
    if ($status -eq "RUNNING") { Write-Host "`n[DONE] Deployed and running!" -ForegroundColor Green; break }
    if ($status -eq "CREATE_FAILED" -or $status -eq "DELETE_FAILED") { Write-Host "`n[FAIL] Deployment failed! Check the App Runner console for the failure event/logs." -ForegroundColor Red; exit 1 }
